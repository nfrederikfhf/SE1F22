...
Scenario: A worker adds an event to register hours
    ...
    And the worker "JHM"'s eventlist does not have any events from "08:00" to "17:00"
    When the worker "JHM" registers working hours from "08:00" to "17:00"
    Then an event labeled "work" is added to the worker "JHM"'s eventlist
  ...
  Scenario: A worker adds an event that overlaps with another events timeframe
    ...
    And the activity with name "Activity 1" is contained in project "22001"'s list of activities
    And there exists an activity with title "Make cucumber files"
    And the worker "JHM"'s has an event from "12:00" to "14:00"
    When the worker "JHM" registers working hours from "08:00" to "17:00"
    Then the event labeled "work" from "08:00" to "17:00" is not registered to the worker "JHM"'s eventlist
